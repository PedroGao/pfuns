/* 虚拟机的本质是： 定义中间代码（如：JAVA 字节码），然后通过 c/c++ 来执行字节码，就和计算器一样，通过输入的 + - \* / 来实现四则运算
   但本质上，虚拟机的执行仍由 c/c++ 程序来完成，VM 执行模拟了一个 CPU，这个模拟的 CPU 只能运行中间代码
*/
...
/* Memory Storage  模拟内存 本质上来说，JAVA 的堆内存也是由 C 语言直接开辟的，所以会有那么的堆参数 */
/* 65536 locations */
